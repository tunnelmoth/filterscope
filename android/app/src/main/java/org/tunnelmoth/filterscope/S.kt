...
object S {
    ...
    val tabOverview get() = p("Overview", "Özet"); val tabSites get() = p("Sites", "Siteler"); val tabEgress get() = p("Egress", "Dışa erişim"); val tabHistory get() = p("History", "Geçmiş")
    ...
    val noProbes get() = p("no probes yet", "henüz test sonucu yok"); val noHistory get() = p("no history yet. Run a scan.", "henüz geçmiş yok. Bir tarama yap.")
    ...
    fun verdict(level: String, n: Int, conf: String) = p("${levelName(level).uppercase()} filtering · $n signals · confidence $conf",
        "${levelName(level).uppercase(java.util.Locale("tr"))} filtreleme · $n bulgu · güven $conf")
    ...
    fun done(s: Int, n: Int) = p("done in $s s, $n signals", "$s s'de bitti, $n bulgu")
    fun probing(a: Int, b: Int) = p("probing $a/$b", "test ediliyor $a/$b")
    fun recheck(n: Int) = p("re-checking $n positives…", "$n bulgu yeniden kontrol ediliyor…")
    fun update(v: String) = p("v$v is available. Tap to download.", "v$v çıktı. İndirmek için dokun.")
    val vendor get() = p("vendor signature", "filtre ürünü imzası"); val score get() = p("score", "skor"); val blocks get() = p("blocks", "engel")
    ...
    val ports get() = p("TCP ports (egress)", "TCP portları (dışa erişim)"); val udp get() = p("UDP egress", "UDP dışa erişimi")
    val download get() = p("download from the service's CDN", "servisin CDN'inden indirme"); val baseline get() = p("baseline", "referans")
